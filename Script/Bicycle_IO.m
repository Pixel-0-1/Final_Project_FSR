...
for i = 2:num_waypoints+1
    ...
    % Perpendicular vector to the main direction
...
% alpha_theta = k - 3*theta_f; Used for theta not tangent
% beta_theta = k + 3*theta_i;
...
while bounds_unsatisfied
    ...
    % Used for theta not tangent
    % theta_flat = s.^3.*theta_f - (s-1).^3.*theta_i + alpha_theta.*s.^2.*(s-1) + beta_theta.*s.*(s-1).^2;
    % theta_flat_dot = 3.*s.^2.*theta_f - 3.*(s-1).^2.*theta_i + 2.*alpha_theta.*s.*(s-1) + alpha_theta.*s.^2 + beta_theta.*(s-1).^2 + 2.*beta_theta.*s.*(s-1);
    
    % theta = theta_flat;
    
    % w_tilde = theta_flat_dot;
